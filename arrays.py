...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        ...
        # Check that value to be stored is valid
        if not isinstance(val, int) or not self._min_val <= val <= self._max_val:
            raise TypeError('Value must be an integer between {self._min_val} and {self._max_val}')
        ...
        if self._resmem:
            ...
            if index <= (self._size - 2):
                new_resmem.copy(self._resmem, source_index=index*self._bytes_per_element, destination_index=(index+1)*self._bytes_per_element)
    ...
    def remove(self, index:int) -> int:
        """
        Remove an element from the array given its index and return its value.
        Raises an IndexError if the index provided is not between the bounds of the array.
        ...
        # Get the removed element's value
        ...
        # Find out the need for reserved memory
        if self._size > 0:
            # If the new size is still bigger than 0, reserve a new memory area with the new size.
            # It is _bytes_per_element smaller than the current one
            new_resmem = ReservedMemory(self._size * self._bytes_per_element)
            # And copy the old memory area
            new_resmem.copy(self._resmem, count=self._size * self._bytes_per_element)
        else:
            # If the new size is 0, there is no need to reserve memory for it
            new_resmem = None

        # Make the new memory area the current one
        self._resmem = new_resmem
...
array = IntArray(bytes_per_element=4)
for i in range(6):
    array.append(i*10000*((-1)**i))
array.insert(5, 10)
print(array)